...
export default function Home() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (marqueeRef.current) {
        const tw = gsap.to(marqueeRef.current, { xPercent: -50, duration: 40, ease: "none", repeat: -1 });
        return () => tw.kill();
      }
    });
    return () => ctx.revert();
  ...
  return (
    <div data-testid="home-page">
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-8 grid md:grid-cols-12 gap-8 items-center min-h-[80vh] relative">
          <div className="md:col-span-6 lg:col-span-7">
            ...
            <div ref={heroStats} className="flex flex-wrap items-center gap-6 mt-12 pt-8 border-t border-cream/80">
              ...
              <div>
                <div className="font-serif text-3xl text-ink">10K+</div>
                <div className="text-xs tracking-[0.16em] uppercase text-inkmuted">Smiles</div>
              </div>
              <div className="w-px h-10 bg-cream"/>
              ...
              <div className="relative rounded-[2.5rem] overflow-hidden aspect-[4/5] shadow-[0_30px_80px_-20px_rgba(139,111,86,0.45)]">
                ...
                <div className="absolute inset-x-0 bottom-0 flex items-center justify-between px-5 py-4 bg-gradient-to-t from-ink/65 to-transparent">
                  <div>
                    ...
                    <div className="font-serif text-lg text-bgmain">{gallery[heroIndex]?.caption || "Smile Saver Dental Clinic"}</div>
                  </div>
                  {gallery.length > 1 && (
                    <div className="flex items-center gap-2">
                      <button
      ...
      {/* MARQUEE STRIP */}
      <section className="overflow-hidden bg-ink text-bgmain py-6 border-y border-cocoa/30">
        <div ref={marqueeRef} className="flex gap-12 whitespace-nowrap will-change-transform">
          {Array.from({length: 2}).map((_, k) => (
            <div key={k} className="flex gap-12 flex-shrink-0">
              {["NABH Accredited", "ISO 9001:2015", "Nobel Biocare Certified", "Straumann Implants", "Zoom Whitening", "Invisalign Provider", "Single-Sitting RCT", "Painless Dentistry"].map(t => (
      ...
      <section className="py-24 px-5 md:px-8" ref={statsRef}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-16">
            {[
              ...
              { icon: Award, t: "16+ Years of Mastery", d: "Founder-led care by Dr. Prateek Aggarwal. Every procedure performed by a focused specialist — never a 'jack of all trades' approach." },
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                { n: 16, suffix: "+", label: "Years of Practice" },
                { n: 10000, suffix: "+", label: "Smiles Crafted" },
