...
import { createBooking, fetchBookedSlots } from "../lib/api";
...
export default function BookingStepper({ services, defaultService, onComplete }) {
  ...
  useEffect(() => {
    ...
    fetchBookedSlots(data.preferred_date)
      .then((res) => setBookedSlots(res.booked_slots || []))
      .catch(() => setBookedSlots([]));
  }, [data.preferred_date]);


  const phoneValid = !data.phone || /^\d{10}$/.test(data.phone.replace(/\D/g, ""));
  const emailValid =
    !data.email ||
          ...
          return (
            <React.Fragment key={s.id}>
              <div className="flex flex-col items-center gap-2 flex-shrink-0">
                <div className={`w-11 h-11 rounded-full flex items-center justify-center transition-all ${complete ? "bg-ink text-bgmain" : active ? "bg-cocoa text-bgmain ring-4 ring-cream" : "bg-beige text-inkmuted"
                  }`}>
            ...
            <div className="grid sm:grid-cols-2 gap-3 max-h-[380px] overflow-y-auto pr-2">
              {services?.map(s => (
                <button
                  ...
                  className={`text-left px-5 py-4 rounded-2xl border transition-all ${data.service === s.title ? "bg-ink text-bgmain border-ink" : "bg-bgmain border-cream hover:border-mocha"
                    }`}
                >
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2.5">
              {TIME_SLOTS.map(t => {
                ...
                const past = isPastSlot(data.preferred_date, t);
                const noDate = !data.preferred_date;
                const disabled = noDate || booked || past;
                ...
                return (
                  <button
                    ...
                    type="button"
                    disabled={disabled}
                    onClick={() => !disabled && set("preferred_time", t)}
                    data-testid={`time-${t.replace(/\s|:/g, '')}`}
                    className={`px-3 py-3 rounded-xl border text-sm transition-all ${noDate
                      ...
                      : booked
                        ...
                        : past
                          ...
                          : data.preferred_time === t
                            ? "bg-ink text-bgmain border-ink"
                            : "bg-bgmain border-cream hover:border-mocha"
                      }`}
                  >
                    {t}
                    {booked && (
